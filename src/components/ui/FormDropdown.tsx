...
function FormDropdown({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div className="absolute top-full mt-px -left-px -right-px z-30 max-h-50 overflow-y-auto scrollbar-none bg-(--bg-secondary) border border-(--bg-border)">
          {options.map((option) => (
            <div
              key={option}
              onClick={() => onChange(option)}
              className="w-full h-10 flex items-center justify-start px-6 py-2.5 font-consolas font-normal text-sm text-(--text-primary) leading-normal border border-(--bg-border) hover:bg-[#1B1B1B]"
            >
              {option}
            </div>
          ))}
